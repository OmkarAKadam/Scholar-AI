...
export default function ResumeRoaster() {
  ...
  return (
    <div className="min-h-screen bg-neutral-50 flex flex-col items-center">
      <div className="w-full bg-resume text-white py-12 px-4 text-center">
        ...
        {!result && !isLoading ? (
          <div className="space-y-8">
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-neutral-200">
              ...
              {resumeText && (
                <div className="mt-4">
                  ...
                  {showTextPreview && (
                    <div className="mt-2 p-4 bg-neutral-50 rounded-lg text-xs text-neutral-600 font-mono max-h-48 overflow-y-auto border border-neutral-200">
                      {resumeText}
                    </div>
                  )}
                </div>
            ...
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-neutral-200">
              ...
              <p className="text-sm text-neutral-500 mb-4 ml-10">Adding a JD unlocks keyword matching and a compatibility score.</p>
              <div className="ml-10">
                <textarea 
                  className="w-full p-4 border border-neutral-300 rounded-xl focus:ring-2 focus:ring-resume outline-none resize-none"
                  rows="4"
          ...
          <div className="bg-white p-16 rounded-2xl shadow-sm border border-neutral-200 h-[600px] flex items-center justify-center">
            <LoadingSpinner message="Aria is reviewing your resume..." colorClass="text-resume" />
          </div>
        ) : result && (
          <div className="space-y-8 animate-fade-in">
            {/* Score Header */}
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-neutral-200 flex flex-col md:flex-row items-center gap-8">
              <div className={`w-40 h-40 rounded-full border-8 flex flex-col items-center justify-center flex-shrink-0 ${getScoreColor(result.score)}`}>
                <span className="text-5xl font-black">{result.score}</span>
                <span className="text-sm font-semibold uppercase">Score</span>
              </div>
              <div className="text-center md:text-left">
                <h2 className="text-3xl font-bold text-neutral-900 mb-2">{result.scoreLabel}</h2>
                {jobDescription && (
                  <div className="inline-flex items-center gap-2 bg-neutral-100 px-3 py-1.5 rounded-lg mb-4">
                    <svg className="w-5 h-5 text-neutral-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
                    <span className="text-sm font-medium text-neutral-700">JD Match: {result.matchPercent}%</span>
                  </div>
                )}
                
                {/* Top Tip */}
                <div className="bg-resume-light border border-resume border-opacity-30 rounded-xl p-4 mt-2">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-neutral-800">{result.topTip}</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {result.missingKeywords && result.missingKeywords.length > 0 && (
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-neutral-200">
                <h3 className="text-lg font-bold text-neutral-900 mb-4">Missing Keywords</h3>
            ...
            {/* Bullet Rewrites */}
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-neutral-200">
              <h3 className="text-xl font-bold text-neutral-900 mb-6">Bullet Point Upgrades</h3>
                ...
                {result.bulletRewrites.map((rewrite, i) => (
                  <div key={i} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="bg-red-50 border border-red-100 p-4 rounded-xl relative">
                      ...
                      <p className="text-sm text-neutral-700 mt-2 line-through opacity-70">{rewrite.original}</p>
                    </div>
                    <div className="bg-green-50 border border-green-100 p-4 rounded-xl relative">
                      <span className="absolute -top-3 left-4 bg-green-100 text-green-800 text-xs font-bold px-2 py-1 rounded">After</span>
                      <p className="text-sm text-neutral-800 mt-2 font-medium">{rewrite.improved}</p>
                    </div>
                  </div>
                ))}
              </div>
